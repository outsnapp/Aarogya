-- Aarogya App Database Schema for Supabase (FINAL FIXED VERSION)
-- This version fixes the RLS policy issue for signup
...
CREATE TABLE public.baby_profiles (
    ...
    delivery_type delivery_type, -- APP CODE USES 'normal' or 'c_section'
...
-- Create RLS Policies (FIXED for signup)
...
-- FIXED: Allow users to insert their own profile during signup
CREATE POLICY "Users can insert own profile" ON public.user_profiles
    FOR INSERT WITH CHECK (auth.uid() = id);

-- FIXED: Allow service role to insert profiles (for signup process)
